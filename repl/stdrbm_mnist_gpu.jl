...
function callback(; rbm, optim, iter, vm, vd, ∂, _...)
    return if iszero(iter % 10)
        push!(grad_history, cpu(∂))
    end
end

optim = Adam(1.0f-4, (0.0f0, 999.0f-3), 1.0f-6)
batchsize = 256
...
training_time = @elapsed begin
    state, ps = pcd!(
        ...
        ϵv = 1.0f-1, ϵh = 0.0f0, l2l1_weights = 0.001, damping = 1.0f-1,
        callback
    )
end
...
let fig = Makie.Figure()
    ...
    Makie.stephist!(ax, vec(cpu(rbm).w), bins = -1:0.001:1, fillto = 1.0e-6, normalization = :pdf)
    Makie.ylims!(ax, 1.0e-3, 1.0e3)
    Makie.resize_to_layout!(fig)
    fig
end
