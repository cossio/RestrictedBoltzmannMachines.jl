...
Every PR receives two automated reviews: a Claude review (`.github/workflows/claude-pr-review.yml`, posting as the `claude` app) and a Codex review (`.github/workflows/codex-pr-review.yml`, posting as the `github-actions` bot). Both post inline comments and end with a formal review verdict — approve, request changes, or comment — and each account holds its own review state, so the two verdicts coexist on the PR. Codex verdict bodies begin with "Codex automated review" and its inline comments carry a `<!-- codex-review -->` marker; Claude prefixes its verdicts with "Claude review:". Each workflow keeps a PR from changing the rules applied to itself: the Claude action self-skips when its workflow file differs from the default branch, and CI then dispatches a fallback review run from the default-branch version via `workflow_dispatch` (a CI-posted comment explains this; if the dispatch fails, comment `@claude review this PR`), while the Codex workflow runs on `pull_request_target`, so the default-branch version always executes and reads REVIEW.md from the base revision. The hosted Codex reviewer (`chatgpt-codex-connector`) may additionally post advisory comment reviews; treat its findings like any other reviewer feedback. When authoring a PR, treat both workflow reviewers' approval as the default bar before merge: address their findings by pushing fixes, or reply in the review threads with reasoning if a finding is mistaken — each reviewer re-runs on every push, and a fresh verdict supersedes that reviewer's previous one.

Reply in every review thread on your PR before considering the work done: either point at the commit that addresses the finding, or explain why it is mistaken. Do not resolve Claude review threads yourself — that reviewer reconciles its own threads on re-runs, resolving them once convinced (by the fix or by your reply) and replying back when not. Codex threads (workflow or hosted) are never reconciled by their reviewer: after replying, resolve those yourself once addressed.
