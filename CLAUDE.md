...
Every PR receives two automated reviews: a Claude review (`.github/workflows/claude-pr-review.yml`) and a Codex review (`.github/workflows/codex-pr-review.yml`). Both post inline comments and end with a review verdict: approve, request changes, or comment. Both submit through the shared github-actions bot account, so verdict bodies are prefixed with the reviewer's name and Codex marks its inline comments with `<!-- codex-review -->`; the bot account's visible review state reflects whichever reviewer submitted an approve/request-changes verdict most recently. PRs that modify the Claude review workflow file cannot run that review directly (the action self-skips when its workflow file differs from the default branch); on such PRs CI dispatches a fallback review run from the default-branch version via `workflow_dispatch`, and a CI-posted comment explains this. If the fallback dispatch fails, comment `@claude review this PR`. The Codex workflow has no such guard and reviews every PR directly, including PRs that modify it. When authoring a PR, treat both reviewers' approval as the default bar before merge: address their findings by pushing fixes, or reply in the review threads with reasoning if a finding is mistaken — each reviewer re-runs on every push, and a fresh verdict supersedes that reviewer's previous one.
