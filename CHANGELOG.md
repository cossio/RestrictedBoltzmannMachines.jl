...
- Added stationarity ("no drift") tests for the samplers ([#120](https://github.com/cossio/RestrictedBoltzmannMachines.jl/issues/120)): chains started *exactly* from the enumerated model distribution `p(v) ∝ exp(-free_energy(rbm, v))` of a small machine must remain at `p(v)` after any number of sweeps, within a Monte-Carlo floor that scales with the number of chains. Unlike convergence tests, this validates the transition kernels independently of mixing time, cleanly separating "the sampler is broken" from "this RBM mixes slowly". Covered: `sample_v_from_v` across the visible (`Binary`/`Spin`/`Potts`/`PottsGumbel`) × hidden (`Gaussian`/`ReLU`/`dReLU`/`pReLU`/`xReLU`/`nsReLU`) layer matrix, for both `RBM` and `StandardizedRBM`, with and without `zerosum`; `metropolis` at `β ≠ 1` against `p_β ∝ exp(-β F)`; `sample_h_from_h` against `free_energy_h`; the first two conditional moments of `sample_h_from_v`; and a companion quadrature check that `free_energy` equals the exact marginalization `-log ∫ dh exp(-energy(rbm, v, h))`.
