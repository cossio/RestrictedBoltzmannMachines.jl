...
- Training and the statistics kernels now use a single weighted code path:
  ...
  - Weights are validated once per training run: they must be finite,
    positive reals. Non-real weights are rejected by the
    `wts::AbstractVector{<:Real}` signatures; non-finite, zero, or negative
    values raise an `ArgumentError` before any mutation; the data must
    contain at least one sample. `initialize!` applies the same validation. Zero-weight samples are thus rejected rather than silently
