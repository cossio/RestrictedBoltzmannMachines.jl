...
@testset "initialization Binary" begin
    ...
    @test RBMs.transfer_mean(rbm.visible) ≈ reshape(Statistics.mean(data; dims=3), size(rbm.visible)) rtol=0.01
end
...
@testset "initialization Potts" begin
    ...
    RBMs.initialize!(rbm, data)
    @test RBMs.transfer_mean(rbm.visible) ≈ reshape(Statistics.mean(data; dims=4), size(rbm.visible)) rtol=0.01
end
