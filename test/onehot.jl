...
@testset "categorical_rand" begin
    ...
    for (s, c) in counts
        @test c ./ sum(values(counts)) ≈ ps[s] atol = 1.0e-2
    end
end
...
@testset "gumbel" begin
    Random.seed!(3)
    @test mean(RBMs.randgumbel() for _ in 1:(10^6)) ≈ MathConstants.γ rtol = 0.01
    @test std(RBMs.randgumbel() for _ in 1:(10^6)) ≈ π / √6 rtol = 0.01
end
