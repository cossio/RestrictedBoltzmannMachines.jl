...
@testset "fields only layers energy ($Layer)" for Layer in (Binary, Spin, Potts)
    layer = Layer(3,4,5)
    x = rand(Bool, size(layer)..., 7)
    @test energy(layer, x) ≈ -RBMs.sum_(layer.θ .* x; dims=(1,2,3))
end
...
@testset "Binary" begin
    ...
    β = rand()
    @test cgf(layer, x) ≈ RBMs.sum_(log1pexp.(layer.θ .+ x); dims=(1,2,3))
    @test all(RBMs.sample_from_inputs(layer, x) .∈ Ref((0, 1)))
    @test gradient(x -> sum(cgf(layer, x)), x)[1] ≈ logistic.(layer.θ .+ x)
    @test gradient(x -> sum(cgf(layer, x, β)), x)[1] ≈ logistic.(β .* (layer.θ .+ x))
    @test mean(RBMs.sample_from_inputs(Binary([0.5]), zeros(1, 10^6))) ≈ logistic(0.5) rtol=0.1
end

@testset "Spin" begin
    layer = Spin(randn(3,4,5))
    x = rand((-1, 1), size(layer)..., 7)
    @test cgf(layer, x) ≈ RBMs.sum_(logaddexp.(layer.θ .+ x, -layer.θ .- x); dims=(1,2,3))
    @test all(RBMs.sample_from_inputs(layer, x) .∈ Ref((-1, 1)))
    @test gradient(x -> sum(cgf(layer, x)), x)[1] ≈ tanh.(layer.θ .+ x)
    β = rand()
    @test gradient(x -> sum(cgf(layer, x, β)), x)[1] ≈ tanh.(β * (layer.θ .+ x))
    @test mean(RBMs.sample_from_inputs(Spin([0.5]), zeros(1, 10^6))) ≈ tanh(0.5) rtol=0.1
end

@testset "Potts" begin
    layer = Potts(randn(3,4,5))
    x = rand((-1, 1), size(layer)..., 7)
    @test cgf(layer, x) ≈ RBMs.sum_(logsumexp(layer.θ .+ x; dims=1); dims=(1,2,3))
    @test all(RBMs.sample_from_inputs(layer, x) .∈ Ref((0, 1)))
    @test all(sum(RBMs.sample_from_inputs(layer, x); dims=1) .== 1)
    β = rand()
    @test gradient(x -> sum(cgf(layer, x, β)), x)[1] ≈ softmax(β .* (layer.θ .+ x); dims=1)
    @test RBMs.mean_(RBMs.sample_from_inputs(Potts([0.5; 0.2]), zeros(2, 10^6)); dims=2) ≈ softmax([0.5; 0.2]; dims=1) rtol=0.1
end

@testset "Gaussian" begin
    layer = Gaussian(randn(3,4,5), randn(3,4,5))
    x = randn(size(layer)..., 7)
    @test energy(layer, x) ≈ RBMs.sum_(@. abs(layer.γ) * x^2 / 2 - layer.θ * x; dims=(1,2,3))
    @test cgf(layer, x) ≈ RBMs.sum_(@. (layer.θ + x)^2 / abs(2layer.γ) + log(2π/abs(layer.γ)) / 2; dims=(1,2,3))
